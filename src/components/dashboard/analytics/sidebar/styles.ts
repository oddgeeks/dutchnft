...
export const SideBarHeaderDropdown = styled.div.attrs({
  className: 'border rounded-lg border-black/10 dark:border-white/10 w-full',
})``;

export const SideBarBody = styled.div.attrs({
  className:
    'border rounded-lg border-black/10 dark:border-white/10 w-full divide-y pb-2',
})``;
...
export const TrackSwitchWrapper = styled.div.attrs<{ className: string }>({
  className: 'flex gap-1',
})``;

export const TrackListWrapper = styled.div.attrs({
  className: 'pb-2 flex flex-col gap-2 min-h-[350px]',
})``;

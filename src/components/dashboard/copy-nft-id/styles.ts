...
export const CopyNFTId = styled.div.attrs({
  className:
    'reltive group rounded-md inline-flex items-center justify-center gap-x-1 px-2.5 py-0.5 w-fit h-fit max-w-full border border-black/10 backdrop-blur hover:bg-black/10 active:bg-black',
})``;
...
export const CopyNFTIdTooltip = styled.div.attrs({
  className:
    'absolute top-[-30px] bg-black/70 rounded-md backdrop-blur px-3 py-1 text-xs text-white',
})``;

...
const DraftNFTCard: React.FC<DraftNFTProps> = ({
  ...
  return (
    <div
      ...
      `}
      style={{ borderColor: `${selected ? 'var(--border-color) !important' : ''}` }}
      onClick={onSelect}>
      {/* unit */}
      <div
        className='absolute top-4 right-0 flex items-center justify-center text-xs font-medium text-white h-5.5 px-1 bg-black/50 rounded-l backdrop-blur dark:bg-white/50 dark:text-dark-surface'
      >
        {amount}
      </div>
      {/* selected mark */}
      {selected && (
        <div
          className='absolute top-4 left-4 flex items-center justify-center w-5 h-5 rounded-full'
        >
          <Icons.ICheckCircle
      ...
      {/* actions */}
      <div
        className='grid grid-cols-2 divide-x divide-black/10 border-t border-black/10 dark:divide-white/10 dark:border-white/10'
      >
        <div
          className='inline-flex items-center justify-center h-9 text-sm font-medium text-black cursor-pointer dark:text-white'
        >Edit</div>
        <div
